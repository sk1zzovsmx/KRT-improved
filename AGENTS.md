...
- **Backward compatibility:** avoid breaking SV keys and existing CLI without a note in this file.

> If you need to change a hard constraint, edit this section and add a note in **15) Change log**.
...
- **Perf:** event‑driven over `OnUpdate`; reuse tables; throttle chat/log spam.
...
!KRT/
  ...
  Libs/                    # vendored libraries (LibStub–based)
    ...
    LibXML-1.0/
```
*(Directory name is `Modules/` in Uppercase.)*
...
## 7) Module map (inside `KRT.lua` unless stated)
...
## 16) Reference templates (optional)

Lua code skeletons in‑repo: If a templates folder exists (e.g., TemplatesLua/), treat it as optional, non‑binding starting points (module skeleton, event listener, CLI sub‑command, logger usage). Prefer existing KRT patterns.

External guide: Lua Programming Development Guide — Agents.md examples collection. https://github.com/gakeez/agents_md_collection/blob/main/examples/lua-programming-development.md

- Report issues with exact client build, steps, and SavedVariables snapshots

---

## 16) Change log (edit by hand)
- _2025‑09‑05_: Initial lightweight version; removed binding “recipes” and explicit `/krtcounts` mention.
