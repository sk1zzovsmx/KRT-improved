...
local function AnnounceRoll(rollType, chatMsg)
	...
	if lootCount >= 1 then
		...
		if rollType == rollTypes.reserved and addon.Reserves and addon.Reserves.FormatReservedPlayersLine then
			...
			message = itemCount > 1 and L[chatMsg .. "Multiple" .. suff]:format(srList, itemLink, itemCount)
				or L[chatMsg]:format(srList, itemLink)
		else
			local suff = addon.options.sortAscending and "Low" or "High"
			message = itemCount > 1 and L[chatMsg .. "Multiple" .. suff]:format(itemLink, itemCount)
...
function Master:BtnHold(btn)
	...
	if lootCount <= 0 or holder == nil then
		return
	end
	countdownRun = false
	local itemLink = GetItemLink()
...
function Master:BtnBank(btn)
	...
	if lootCount <= 0 or banker == nil then
		return
	end
	countdownRun = false
	local itemLink = GetItemLink()
...
function Master:BtnDisenchant(btn)
	...
	if lootCount <= 0 or disenchanter == nil then
		return
	end
	countdownRun = false
	local itemLink = GetItemLink()
...
function UpdateDropDowns(frame)
	if not frame or not KRT_CurrentRaid then
		return
	end
...
function addon:TRADE_ACCEPT_UPDATE(tAccepted, pAccepted)
	...
	if itemCount == 1 and trader and winner and trader ~= winner then
		if tAccepted == 1 and pAccepted == 1 then
			addon:Debug(
				"DEBUG",
				"Trade confirmed. Logging item trade: %s to %s",
				tostring(currentRollItem),
				tostring(winner)
			)
...
function AssignItem(itemLink, playerName, rollType, rollValue)
	addon:Debug(
		"DEBUG",
		"Attempting to assign item %s to %s with rollType %s and rollValue %s",
		itemLink,
		playerName,
		tostring(rollType),
		tostring(rollValue)
...
function TradeItem(itemLink, playerName, rollType, rollValue)
	addon:Debug(
		"DEBUG",
		"Trading item %s to %s, rollType=%s, rollValue=%s",
		itemLink,
		playerName,
		tostring(rollType),
		tostring(rollValue)
