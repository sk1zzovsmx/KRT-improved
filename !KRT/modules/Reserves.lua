...
function Reserves:ParseCSV(csv)
	...
	for line in csv:gmatch("[^\r\n]+") do
		if firstLine then
			...
			local _, itemIdStr, source, playerName, class, spec, note, plus =
				line:match('^"?(.-)"?,(.-),(.-),(.-),(.-),(.-),(.-),(.-)')
			...
			if normalized and itemId then
				...
				reservesData[normalized] = reservesData[normalized]
					or {
						original = playerName,
						reserves = {},
					}
				...
				for _, entry in ipairs(reservesData[normalized].reserves) do
					if entry.rawID == itemId then
						...
						addon:Debug(
							"DEBUG",
							"Updated quantity for player %s, item ID %d. New quantity: %d",
							playerName,
							itemId,
							entry.quantity
						)
...
function Reserves:GetReserveCountForItem(itemId, playerName)
	...
	for _, r in ipairs(entry.reserves or {}) do
		if r.rawID == itemId then
			addon:Debug(
				"DEBUG",
				"Found reserve for itemId: %d, player: %s, quantity: %d",
				itemId,
				playerName,
				r.quantity
			)
...
function Reserves:RefreshWindow()
	...
	for index, entry in ipairs(displayList) do
		...
		if not seenSources[source] then
			...
			local prefix = collapsedBossGroups[source] and "|TInterface\\Buttons\\UI-PlusButton-Up:12|t "
			...
			headerBtn:SetScript("OnClick", function()
				...
				addon:Debug(
					"DEBUG",
					"Toggling collapse state for source: %s to %s",
					source,
