	...
	if current then
		if current.zone == instanceName then
			...
			if current.size ~= desiredSize then
				addon:Debug(
					"INFO",
					"Raid size changed from %d to %d. Creating new raid session.",
					current.size,
					desiredSize
				)
...
function Raid:AddPlayer(t, raidNum)
	...
	if not raidNum or not t or not t.name then
		addon:Debug(
			"ERROR",
			"Raid:AddPlayer called with invalid parameters (raidNum=%s, player=%s)",
			tostring(raidNum),
...
function Raid:AddBoss(bossName, manDiff, raidNum)
	...
	if not raidNum or not bossName then
		addon:Debug(
			"ERROR",
			"Raid:AddBoss called with invalid parameters (raidNum=%s, bossName=%s)",
			tostring(raidNum),
			tostring(bossName)
		)
		return
	...
	addon:Debug(
		"INFO",
		"Boss kill recorded: '%s' [Difficulty %d] in Raid #%d with %d players. Time: %s",
		bossName,
		instanceDiff,
		raidNum,
		#players,
...
function Raid:AddLoot(msg, rollType, rollValue)
	...
	local _, _, _, _, itemId = string.find(
		itemLink,
	...
	if itemRarity and itemRarity < lootThreshold then
		addon:Debug(
			"DEBUG",
			"Ignored item %s [%d] due to rarity (%d < %d)",
			itemName or "unknown",
			itemId or 0,
			itemRarity or -1,
			lootThreshold
	...
	addon:Debug(
		"INFO",
		"Loot added: %s x%d to %s [Raid #%d, Boss #%d, Rarity: %d, Roll: %s %s]",
		itemName or "unknown",
		itemCount,
		player,
		KRT_CurrentRaid,
...
function Raid:Expired(rID)
	...
	addon:Debug(
		"DEBUG",
		"Expired: Raid #%d started at %d, now %d, duration %d => %s",
		rID,
		startTime,
		currentTime,
		validDuration,
		isExpired and "EXPIRED" or "ACTIVE"
	)
	return isExpired
end
...
function Raid:GetLootID(itemID, raidNum, holderName)
	...
	for k, v in ipairs(loot) do
		if v.itemId == itemID and v.looter == holderName then
			...
			addon:Debug(
				"DEBUG",
				"GetLootID: Found item %d looted by '%s' at position %d (Raid #%s)",
				itemID,
				holderName,
				pos,
	...
	if pos == 0 then
		addon:Debug(
			"DEBUG",
			"GetLootID: Item %d looted by '%s' not found in Raid #%s",
			itemID,
			holderName,
			tostring(raidNum or KRT_CurrentRaid)
		)
...
function Raid:CheckPlayer(name, raidNum)
	...
	addon:Debug(
		"DEBUG",
		"CheckPlayer: %s (normalized to %s) found in raid %s: %s",
		originalName,
		name,
		tostring(raidNum or KRT_CurrentRaid),
		tostring(found)
...
function Raid:GetPlayerName(id, raidNum)
	...
	addon:Debug(
		"DEBUG",
		"GetPlayerName: id=%s, raid=%s, resultName=%s",
		tostring(id),
		tostring(raidNum),
		tostring(name)
	)
...
function Raid:GetPlayerLoot(name, raidNum, bossNum)
	...
	addon:Debug(
		"DEBUG",
		"GetPlayerLoot: original=%s, resolvedName=%s, raid=%s, boss=%s, itemsFound=%d",
		tostring(originalName),
		tostring(name),
		tostring(raidNum or KRT_CurrentRaid),
		tostring(bossNum or "all"),
		#items
	)
	return items
end
...
function addon:GetPlayerRank(name, raidNum)
	...
	addon:Debug(
		"DEBUG",
		"GetPlayerRank: original=%s, resolvedName=%s, raid=%s, rank=%d",
		tostring(originalName),
		tostring(name),
		tostring(raidNum or KRT_CurrentRaid),
		rank
...
function addon:GetPlayerClass(name)
	...
	addon:Debug(
		"DEBUG",
		"GetPlayerClass: name=%s, realm=%s, class=%s",
...
function addon:IsMasterLooter()
	...
	addon:Debug(
		"DEBUG",
		"IsMasterLooter: method=%s, partyID=%s => %s",
		tostring(method),
		tostring(partyID),
		tostring(isML)
	)
