...
function Raid:AddPlayer(t, raidNum)
	...
	if not raidNum or not t or not t.name then
		addon:Debug("ERROR", "Raid:AddPlayer called with invalid parameters (raidNum=%s, player=%s)", tostring(raidNum),
			tostring(t and t.name))
		return
	end
...
function Raid:AddBoss(bossName, manDiff, raidNum)
	...
	if not raidNum or not bossName then
		addon:Debug("ERROR", "Raid:AddBoss called with invalid parameters (raidNum=%s, bossName=%s)", tostring(raidNum),
			tostring(bossName))
		return
	end
...
function Raid:AddLoot(msg, rollType, rollValue)
	...
	addon:Debug("INFO", "Loot added: %s x%d to %s [Raid #%d, Boss #%d, Rarity: %d, Roll: %s %s]", itemName or "unknown",
		itemCount, player, KRT_CurrentRaid, KRT_LastBoss or 0, itemRarity or -1, tostring(rollType), tostring(rollValue))
end
...
function Raid:GetLootID(itemID, raidNum, holderName)
	...
	for k, v in ipairs(loot) do
		if v.itemId == itemID and v.looter == holderName then
			pos = k
			addon:Debug("DEBUG", "GetLootID: Found item %d looted by '%s' at position %d (Raid #%s)", itemID, holderName,
				pos, tostring(raidNum or KRT_CurrentRaid))
			break
		end
	end
	if pos == 0 then
...
function Raid:GetPlayerLoot(name, raidNum, bossNum)
	...
	addon:Debug("DEBUG", "GetPlayerLoot: original=%s, resolvedName=%s, raid=%s, boss=%s, itemsFound=%d",
		tostring(originalName), tostring(name), tostring(raidNum or KRT_CurrentRaid), tostring(bossNum or "all"), #items)
	return items
end
