...
do
	...
	local function OpenItemMenu(btn)
		if not addon.Logger.selectedItem then
			return
		end
		...
		local menuList = {
			...
			},
			{
				...
				func = function()
					StaticPopup_Show("KRTLOGGER_ITEM_EDIT_ROLL")
				end,
			},
			{
...
do
	...
	function UpdateUIFrame(self, elapsed)
		...
		if Utils.periodic(self, frameName, updateInterval, elapsed) then
			...
			Utils.enableDisable(
				...
				(
					selectedRaid
					and selectedRaid ~= KRT_CurrentRaid
					and not addon.Raid:Expired(selectedRaid)
					and addon:GetRaidSize() == KRT_Raids[selectedRaid].size
				)
			)
	...
	function Raids:SetCurrent(btn)
		if not btn or not selectedRaid or not KRT_Raids[selectedRaid] then
			return
		end
	...
	do
		...
		function Raids:Sort(t)
			if t == nil or sortTypes[t] == nil then
				return
			end
...
do
	...
	function Boss:Edit(btn)
		if not btn or not selectedBoss then
			return
		end
	...
	do
		local function DeleteBoss()
			...
			if not raid or not raid.bossKills[selectedBoss] then
				return
			end
...
do
	...
	do
		local function DeleteAttendee()
			if not selectedBoss or not selectedBossPlayer then
				return
			end
			local raid = KRT_Raids[selectedRaid]
			if not raid or not raid.bossKills[selectedBoss] then
...
do
	...
	do
		local function DeleteAttendee()
			...
			local raid = KRT_Raids[selectedRaid]
			if not raid or not raid.bossKills[selectedPlayer] then
				return
			end
...
do
	...
	do
		...
		local sortTypes = {
			...
			type = function(a, b)
				if not a.rollType or not b.rollType then
					return
				end
				if ascending then
			...
			roll = function(a, b)
				if not a.rollValue and not b.rollValue then
					return
				end
				if ascending then
...
do
	...
	function BossBox:Fill()
		...
		if not selectedRaid or not selectedBoss then
			return
		end
