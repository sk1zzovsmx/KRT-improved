...
local function AddRoll(name, roll, itemId)
	...
	if itemId then
		...
		addon:Debug(
			"DEBUG",
			"Updated itemRollTracker: itemId=%d, player=%s, count=%d",
			itemId,
			name,
...
function addon:RollStatus()
	addon:Debug(
		"DEBUG",
		"RollStatus queried: type=%s, record=%s, canRoll=%s, rolled=%s",
		tostring(currentRollType),
		tostring(record),
		tostring(canRoll),
		tostring(rolled)
	)
...
function addon:DidRoll(itemId, name)
	...
	addon:Debug(
		"DEBUG",
		"DidRoll: name=%s, itemId=%d, used=%d, allowed=%d, result=%s",
		name,
		itemId,
		used,
		allowed,
...
function addon:IsValidRoll(itemId, name)
	...
	addon:Debug(
		"DEBUG",
		"IsValidRoll: %s on item %d: used=%d, allowed=%d, valid=%s",
		name,
		itemId,
		used,
		allowed,
