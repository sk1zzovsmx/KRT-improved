...
-- events: none
-- notes: pure Master roll row display models
...
function RollRows.BuildModel(opts)
    ...
    for i = 1, #baseRows do
        ...
        if row then
            isSelected = selectedNames[row.name] == true or (selectionState.singleWinnerSelected and selectionState.winnerName == row.name)
