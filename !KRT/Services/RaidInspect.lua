...
local GetTime = GetTime or function()
    return 0
end
local UnitGUID = UnitGUID or noop
local UnitExists = UnitExists or noop
local UnitIsConnected = UnitIsConnected or noop
local CanInspect = CanInspect or noop
local CheckInteractDistance = CheckInteractDistance or noop
local NotifyInspect = NotifyInspect or noop
local ClearInspectPlayer = ClearInspectPlayer or noop
local UnitAffectingCombat = UnitAffectingCombat or noop
local GetInventoryItemLink = GetInventoryItemLink or noop
...
local function finalizeRequest(raidId, playerNid, status, reason, unit, request)
    ...
    if snapshotStatus == "ready" or snapshotStatus == "skipped" or snapshotStatus == "timeout" or snapshotStatus == "failed" then
...
function module:GetPersistedSnapshot(raidOrId, playerNid)
    ...
    return inspect.players and inspect.players[nid]
end

function module:GetRuntimeStatus(raidId, playerNid)
    local rid = normalizeRaidIndex(raidId)
    local nid = normalizePlayerNid(playerNid)
    if not rid or nid <= 0 then
        return nil
    end
    local runtime = runtimeStatusByRaid[rid]
    return runtime and runtime[nid]
end

function module:ClearQueue(raidId)
    local rid = normalizeRaidIndex(raidId)
    if rid then
...
function module:HandleInspectReady(guid)
...
function module:HandlePlayerRegenEnabled()
...
local inspectReadyEvent = Events.GetWowForwarded("INSPECT_TALENT_READY")
if Bus and Bus.RegisterCallback and inspectReadyEvent then
    Bus.RegisterCallback(inspectReadyEvent, function(_, guid)
        module:HandleInspectReady(guid)
    end)
end

local playerRegenEnabledEvent = Events.GetWowForwarded("PLAYER_REGEN_ENABLED")
if Bus and Bus.RegisterCallback and playerRegenEnabledEvent then
    Bus.RegisterCallback(playerRegenEnabledEvent, function()
        module:HandlePlayerRegenEnabled()
    end)
end
