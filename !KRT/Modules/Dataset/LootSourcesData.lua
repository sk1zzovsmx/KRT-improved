...
local function buildSourceKey(raidName, source)
    ...
    return tconcat({ raidKey, kind, tostring(npcId), sourceName, LootSourceCandidates.GetModeSignature(source and source.modes) }, "|")
end
    ...
    if type(candidate.modes) == "table" then
        for mode in pairs(candidate.modes) do
            ...
            if difficulty and size then
                ...
                local byItemIdByFallback = getOrCreateModeSizeBucket(raidEntry, UNKNOWN_MODE_DIFFICULTY, UNKNOWN_MODE_SIZE, itemId)
