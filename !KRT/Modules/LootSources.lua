...
local function getIndexedCandidates(itemId, context, modeKey)
    ...
    local raidKey = normalizedContext.normalizedRaid or normalizedContext.normalizedZoneName or normalizedContext.normalizedInstanceName
    if not raidKey then
        return nil
    end
...
function LootSources.FindSource(itemId, context)
    ...
    local candidates = filterCandidates(LootSources.GetCandidates(numericItemId, normalizedContext, modeKey), normalizedContext, modeKey)
