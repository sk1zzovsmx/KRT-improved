...
Diag.D.LogRaidCheckInitialRaidState = "[Raid] CheckInitialRaidState inGroup=%s currentRaid=%s instance=%s type=%s diff=%s"
...
Diag.D.LogLoggerLootLogAttempt = "[Logger] loot entry attempt source=%s raidId=%s lootNid=%s " .. "looter=%s type=%s roll=%s lastBoss=%s"
...
Diag.D.LogLoggerLootBefore = "[Logger] loot entry BEFORE raidId=%d lootNid=%s link=%s " .. "prevLooter=%s prevType=%s prevRoll=%s"
...
Diag.E.LogLoggerVerifyFailed = "[Logger] loot entry VERIFY FAILED raidId=%d lootNid=%s " .. "got(looter=%s type=%s roll=%s)"
