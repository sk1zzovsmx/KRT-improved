...
assertContains(configLua, "local UI = feature.UI", "Config widget must localize UI root from feature shared")
assertContains(configLua, "local Layout = UI.Layout", "Config widget must localize Layout from UI root")
...
assertContains(configLua, "Layout.ApplyRows", "Config widget must use the shared UI.Layout renderer")
...
assertContains(optionsLayoutLua, "function Layout.CheckRow", "OptionsLayout must expose declarative checkbox row specs")
assertContains(optionsLayoutLua, "function Layout.CommandRow", "OptionsLayout must expose declarative command row specs")
assertContains(optionsLayoutLua, "function Layout.EditCommandRow", "OptionsLayout must expose declarative edit-command row specs")
assertContains(optionsLayoutLua, "function Layout.DropDownRow", "OptionsLayout must expose declarative dropdown row specs")
...
assertContains(configLua, 'Layout.CommandRow("DefaultsPreset", "DefaultsBtn"', "Master Loot Defaults preset must use the fixed command column")
assertContains(configLua, 'Layout.CommandRow("QuietPreset", "QuietPresetBtn"', "Master Loot Quiet preset must use the fixed command column")
assertContains(configLua, 'Layout.CommandRow("StandardPreset", "StandardPresetBtn"', "Master Loot Standard preset must use the fixed command column")
assertContains(configLua, 'Layout.CommandRow("VerbosePreset", "VerbosePresetBtn"', "Master Loot Verbose preset must use the fixed command column")
...
assertContains(raidLootMethodLua, "RequestGroupLootRestorePrompt", "Loot-method service must request the UI-owned Group Loot restore prompt")
assertNotContains(raidLootMethodLua, "KRT_CONFIRM_GROUP_LOOT_RESTORE", "Loot-method service must not define the Group Loot restore popup")
assertContains(masterLua, "KRT_CONFIRM_GROUP_LOOT_RESTORE", "Master controller must define the Group Loot restore popup")
assertContains(masterLua, "Popups.ShowConfirm", "Master controller must show the Group Loot restore popup through the shared popup helper")
assertContains(uiFramesLua, "function Popups.ShowConfirm", "Shared popup helper must define and show confirm popups")
assertContains(uiFramesLua, "function Popups.Show(key, textArg1, textArg2, data)", "Shared popup helper must own StaticPopup_Show calls")
assertContains(uiFramesLua, "StaticPopup_Show(key, textArg1, textArg2, data)", "Shared popup helper must pass popup data through StaticPopup_Show")
assertContains(uiFramesLua, "function Popups.Hide", "Shared popup helper must own StaticPopup_Hide calls")
assertContains(uiFramesLua, "function Popups.Resize", "Shared popup helper must own StaticPopup_Resize calls")
assertContains(uiFramesLua, "button3 = options.button3", "Shared popup helper must support three-button confirm popups")
assertContains(uiFramesLua, "OnCancel = options.onCancel", "Shared popup helper must support confirm cancel handlers")
assertContains(uiFramesLua, "function Popups.ShowEditBox", "Shared popup helper must show standard editbox popups")
assertContains(uiFramesLua, "Popups.DefineEditBox(key, text, onAccept, onShow, validate)", "Shared editbox popup helper must define missing editbox popups")
assertContains(configLua, "Popups.ShowConfirm", "Config widget must use the shared popup helper for confirm popups")
assertNotContains(configLua, "StaticPopupDialogs", "Config widget must not define StaticPopup dialog tables directly")
assertNotContains(configLua, "StaticPopup_Show", "Config widget must not call StaticPopup_Show directly")
...
assertContains(
    ...
    "Master Loot panel must render the notice duration as an editbox row"
)
...
assertContains(configLua, 'Layout.DropDownRow("LoggerLootQuality", "LoggerLootQualityDropDown"', "Loot History quality selector must use a fixed dropdown row")
assertContains(configLua, "LoggerLootQualityDropDown", "Loot History dropdown must be assigned to the fixed command column")
assertContains(configLua, 'Layout.EditCommandRow("RequireDatabase"', "Loot History sync target commands must use editbox command rows")
assertContains(configLua, 'Layout.EditCommandRow("RequireDatabase", "RequireDatabaseEditBox"', "Require Database editbox must be assigned to the fixed command column")
assertContains(configLua, 'Layout.EditCommandRow("PushDatabase", "PushDatabaseEditBox"', "Push Database editbox must be assigned to the fixed command column")
...
assertContains(configLua, 'Layout.ButtonRow({ "DeleteBtn", "CancelBtn" }', "Cleanup popup buttons must use a stable button row")
...
assertContains(configLua, 'Layout.ButtonRow({ "RefreshPreviewBtn", "ClearPreviewBtn" }', "LFM Spam preview buttons must share one fixed row")
assertContains(configLua, 'Layout.ButtonRow({ "OpenBtn", "StartBtn", "StopBtn" }', "LFM Spam action buttons must share one fixed row")
...
assertContains(configLua, 'Layout.ButtonRow({ "PreviewBtn", "OpenBtn" }', "Raid Warning template buttons must share one fixed row")
assertContains(configLua, 'Layout.CommandRow("ClearSaved", "ClearSavedBtn"', "Raid Warning clear command must use the fixed command column")
assertContains(configLua, "ClearSavedDesc", "Raid Warning clear command must keep text in the left column")
